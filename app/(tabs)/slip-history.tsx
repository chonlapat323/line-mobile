...
function LineStatusBadge({ status }: { status?: string | null }) {
  ...
  if (status === "sent") return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 3 }}>
      ...
      <Text style={{ fontSize: 11, color: "#16a34a" }}>ส่ง LINE แล้ว</Text>
    </View>
  );
  return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 3 }}>
      <Ionicons name="close-circle" size={12} color={colors.error} />
      <Text style={{ fontSize: 11, color: colors.error }}>LINE ล้มเหลว</Text>
    </View>
  );
}
...
const st = StyleSheet.create({
  ...
  shopName: { fontSize: 14, fontWeight: "700", color: colors.textPrimary, flex: 1 },
...
const md = StyleSheet.create({
  ...
  value: { fontSize: 13, color: colors.textPrimary, fontWeight: "600", textAlign: "right", flex: 1, marginLeft: 16 },
});
